      ...
      <div className="max-w-5xl mx-auto px-4 py-16 space-y-24">
        ...
        <div className="rounded-3xl bg-zinc-900/60 border border-white/8 p-8 md:p-14 space-y-8">
          ...
          <div className="space-y-6 text-gray-300 leading-relaxed text-[15px]">
            ...
            <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 px-6 py-5 text-sm leading-relaxed text-gray-300">
              <strong className="text-white">Where camera brands actually differ: the "rendering" step.</strong> Every RAW pipeline starts the same way — correct the sensor, demosaic, white-balance, convert to linear color. What defines a brand's look is the next step, where flat linear data gets pushed through a color rendering curve into a finished image — Hasselblad calls its version HNCS, Fujifilm calls it Film Simulation, Sony calls it Creative Look, and so on. Most current cameras (Sony, Nikon, Canon, and newer Panasonic bodies) can even produce real in-camera HDR stills today. But every manufacturer runs two separate systems to get there: a video-log LUT catalogue (F-Log2, V-Log, N-Log, S-Log3) meant for grading footage in editing software, and a completely different, simpler curve-based renderer for JPEG/HEIF stills — the two aren't the same engine, and a video-log LUT can't just be dropped onto a photo's linear sensor data without producing the wrong contrast and color. raw2hdr bridges that gap instead of avoiding it: it mathematically re-encodes your RAW's linear data into whichever log format a given LUT expects, then applies that same 3D LUT format used industry-wide for color grading — so a grade authored for video can render correctly from a single still photo, with the recovered highlight data carrying through as one continuous signal into a final BT.2100 HLG image. <Link to="/technical-deep-dive#rendering-engines" className="text-blue-400 hover:text-blue-300 underline underline-offset-2">See the full manufacturer-by-manufacturer breakdown →</Link>
            </div>
          </div>
        </div>
