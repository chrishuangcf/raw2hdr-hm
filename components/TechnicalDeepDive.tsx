          ...
          <Section id="rendering-engines" number="03" icon={<Scale className="w-5 h-5 text-rose-400" />}
            ...
            <div className="rounded-2xl bg-zinc-900/60 border border-zinc-800 overflow-hidden">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {[
                      ...
                      ['Panasonic', 'Yes — V-Log/V-Gamut LUTs, distributed for video grading', 'Photo Style — curve/parametric adjustments; LUMIX Lab\'s RAW-import LUT tool uses the same "[LUT Library]" mechanism as in-camera', 'Yes, confirmed — Panasonic\'s own manual states this mechanism defaults to V-Log as the base encoding for any LUT, photo or video'],
              ...
              <div className="px-5 py-3 bg-zinc-900 border-t border-zinc-800">
                <p className="text-xs text-gray-500">Sources: Fujifilm-x.com F-Log2 LUT download page & shopusa.fujifilm-x.com grading guide; Fuji Rumors interview with Fujifilm colour-science managers; Panasonic DC-S9 manual ([LUT Library] / Photo Style pages) & av.jpn.support.panasonic.com V-Log LUT library; Leica-camera.com SL3 L-Log downloads; Sony S-Log technical documentation (pro.sony); Canon Europe 3D LUTs / Cinema EOS explainer; PetaPixel coverage of Fujifilm's F-Log2 Film Simulation LUTs (Nov 2025); Hasselblad HNCS page.</p>
              </div>
            </div>
            ...
            <DetailsPanel title="Panasonic — Photo Style, V-Log, and iDynamic" accent="border-rose-500/20">
              <p>Panasonic is the clearest documented case of the video/stills LUT split actually converging. The DC-S9 manual states its "[LUT Library]" mechanism — the same one LUMIX Lab's RAW-import LUT feature uses, added in the app's v3.0.0 update (May 2026) — reads a "base Photo Style" embedded in each LUT file, and falls back to <strong className="text-white">V-Log as the default base encoding</strong> if a LUT doesn't specify one. That is Panasonic's own documentation confirming V-Log is the expected input format for any LUT run through this mechanism, photo or video — one shared LUT pipeline, not two unrelated systems, which is a stronger convergence than the other five manufacturers surveyed here. (Independent reverse-engineering of the math — sRGB → linear → V-Gamut → V-Log encode → LUT — supports this shape, though Panasonic doesn't publish the pipeline as an explicit stage-by-stage diagram.) Photo Style itself still has separately documented adjustable parameters (contrast, sharpness, saturation/colour, noise reduction), and iDynamic combines a metering-level exposure reduction with a shadow-lifting curve at render — the same exposure-plus-curve category as the other manufacturers. HDR stills (10-bit HEIF, HLG) arrived via firmware specifically for the S1R II, S1 II, and S1 IIE — not the S5 II, S5 IIX, GH7, or G9 II.</p>
            </DetailsPanel>
